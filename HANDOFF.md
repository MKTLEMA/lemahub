...
- Advisors: `log_historico` search_path CORRIGIDO (2026-09-02, `SET search_path = public, extensions`, migration atualizada); tabela órfã `public.lemahubmkt` REMOVIDA (2026-09-02 — confirmada vazia, sem FKs/triggers/views/dependências antes do DROP, sem referência no código). Pendente (usuário): leaked password protection (ativar no dashboard: Authentication → Settings).
